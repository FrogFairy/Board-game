...
class Board:
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                if self.player[y][x] == 2:
                    ...
                    Objects(self.images, 'people', r_r[0][0] + 1, r_r[0][1] + 1, 98)
                    color = background
                    ...
                    pygame.draw.rect(screen, self.grass[0], r_r, border_radius=20)
                    Objects(self.images, 'house', r_r[0][0] + 1, r_r[0][1] + 1, 98)
                    color = background
                else:
    ...
    def on_click(self, cell_coords):
        if cell_coords and cell_coords != 'cancel':
            self.step = []
            x, y = cell_coords
            if len(self.steps) < 10 and self.player[y][x] != 2 and x >= 0 and y >= 0:
...
def error(check, h, w, cell_size, top, left):
    ...
    if check is True:
        ...
        Errors(tile_image, 'house', x1, y1)
        Errors(tile_image, 'house', x2, y2)
    elif check[0] == 'house':
        ...
        x1 = check[1][0] * cell_size + left + 2
        y1 = check[1][1] * cell_size + top + 2
        Errors(tile_image, 'people', x1, y1)
...
def main():
    ...
    board = Board(6, 6)
    board.set_view((width - (6 * 100)) // 2, (height - (6 * 100)) // 2, 100)
    running = True
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
